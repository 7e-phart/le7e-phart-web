...
class ContentService {
  ...
  Future<List<ArticleModel>> getArticles() async {
    print('ContentService.getArticles appelé');
    if (!_isFirebaseAvailable) {
      print('ContentService: Firebase non disponible');
      throw Exception('Firebase non disponible');
    }
    ...
    try {
      print('ContentService: Récupération des articles depuis Firestore...');
      final snapshot = await _firestore!
          .collection('articles')
          .orderBy('createdAt', descending: true)
          .get();
      
      print('ContentService: ${snapshot.docs.length} articles récupérés');
      return snapshot.docs
          .map((doc) {
            print('ContentService: Article - ${doc.id}: ${doc.data()}');
            return ArticleModel.fromMap(doc.data(), doc.id);
          })
          .toList();
    } catch (e) {
      print('ContentService: Erreur lors de la récupération des articles: $e');
      throw Exception('Erreur lors de la récupération des articles: $e');
    }
  }

  Future<void> addArticle(ArticleModel article) async {
    print('ContentService.addArticle appelé avec: ${article.toMap()}');
    if (!_isFirebaseAvailable) {
      print('ContentService: Firebase non disponible');
      throw Exception('Firebase non disponible');
    }
    ...
    try {
      print('ContentService: Ajout de l\'article à Firestore...');
      final docRef = await _firestore!.collection('articles').add(article.toMap());
      print('ContentService: Article ajouté avec ID: ${docRef.id}');
    } catch (e) {
      print('ContentService: Erreur lors de l\'ajout de l\'article: $e');
      throw Exception('Erreur lors de l\'ajout de l\'article: $e');
    }
  }

  Future<void> updateArticle(String articleId, ArticleModel article) async {
    if (!_isFirebaseAvailable) {
      throw Exception('Firebase non disponible');
    }
    ...
    try {
      ...
      throw Exception('Erreur lors de la modification de l\'article: $e');
    }
  }
